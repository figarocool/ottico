#tag Window
Begin Window WinVisualizzaMontature
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   True
   Composite       =   False
   ...
   Placement       =   0
   Resizeable      =   True
   Title           =   "Software Ottico - Gestione Montature"
   Visible         =   True
   Width           =   546
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub ShowMontature(idcliente as integer)
		  ...
		  While not rs.eof
		    ...
		    lstTecnica.cell(lstTecnica.listcount-1,1) = rs.idxField(3).StringValue
	...
	#tag Method, Flags = &h0
		Sub StampaMontatureCliente(idcliente As Integer,VisualizzaNote As Boolean=true)
		  ...
		  Dim rs As RecordSet
		  rs=db.SQLSelect("SELECT * FROM [dati-tecnici-montature] WHERE idcliente="+str(idcliente)+"")
...
#tag Events cmdAggiungi
	#tag Event
		Sub Action()
		  WinMontature.Show
		  WinMontature.idcliente=Idcaricato
	...
	#tag Event
		Sub Action()
		  if lstTecnica.ListIndex <> -1 then
		    WinModificaMontature.Caricadati(Idcaricato,lstTecnica.rowtag(lstTecnica.listindex))
	...
	#tag Event
		Sub Action()
		  if lstTecnica.ListIndex <> -1 then
		    ...
		    if tmp = 6 then
		      db.SQLExecute("delete from [dati-tecnici-montature]  where idcliente="+str(idcaricato)+" and idtecnico="+lstTecnica.RowTag(lstTecnica.ListIndex))
		      db.Commit
		      call lstTecnica.RemoveRow(lstTecnica.ListIndex)
		      MsgBox "Riga Montature Eliminata."
		    end if
		  end if
...
#tag Events cmdStampa
	#tag Event
		Sub Action()
		  ...
		  if result <> 6 then
		    call StampaMontatureCliente(Idcaricato)
		  else
		    call StampaMontatureCliente(Idcaricato,true)
		  end if
